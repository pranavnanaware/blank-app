    ...
    try:
        ...
        with tqdm(total=total_parts, desc="Uploading", unit="part") as pbar:
            while True:
                ...
                status_text.text(f"Uploading part {part_number} of {total_parts}")
                part_number += 1
                pbar.update(1)
...
# Main Streamlit App
def main():
    st.title("Video Upload to S3 (Up to 5 GB)")
    st.write("Select a large video file (up to 5 GB) to upload to your Amazon S3 bucket.")
    ...
    if uploaded_file is not None:
        ...
        st.write(f"**File size**: {file_details['filesize'] / (1024 * 1024 * 1024):.2f} GB")
        ...
        if st.button("Upload to S3"):
            with st.spinner("Uploading..."):
                ...
                with tempfile.TemporaryFile() as tmp_file:
                    ...
                    success = multipart_upload(uploaded_file, s3_client, S3_BUCKET, s3_key)
                    if success:
                        st.success(f"File uploaded successfully to `s3://{S3_BUCKET}/{s3_key}`")
                    else:
